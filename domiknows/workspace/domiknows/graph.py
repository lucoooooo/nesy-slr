...
with Graph(name='global') as graph:
    ...
    """ Aggiunge vincoli in più ma aumenta il calcolo computazionale (se cifre => somma)
    for d0_val in range(config.digitRange):
        for d1_val in range(config.digitRange):
            sum_val = d0_val + d1_val
        
            if sum_val >= config.summationRange:
                continue
