...
# Parameters
parser.add_argument("--epochs_NSIL", type=int, default=1)
...
parser.add_argument("--pct_nesy", type=int, default=100)
...
epochs_NSIL = args.epochs_NSIL
num_it = args.num_iterations
pct_nesy = args.pct_nesy

class CFG:
    def __init__(self, epochs_NSIL, seed, batch_size_train,num_workers, num_it, pct):
        self.epochs = epochs_NSIL
        self.seed = seed
...
cfg = CFG(epochs_NSIL,seed,batch_size_train,num_workers, num_it, pct_nesy)
...
train_loader_b_ns, test_loader_b_ns = utils.mnist_sum_2_loader(
    data_dir, batch_size_train, batch_size_test, "basic",
    train_dataset=train_dataset, test_dataset=test_dataset
)
train_loader_b0_ns, test_loader_b0_ns = utils.mnist_sum_2_loader(
    data_dir, batch_size_train, batch_size_test, "mnistb0",
    train_dataset=train_dataset, test_dataset=test_dataset
)
train_loader_b3_ns, test_loader_b3_ns = utils.mnist_sum_2_loader(
...
b0_sym = utils.MNISTNet_b0_sym()
b0_nsym = utils.MNISTNet_b0()
b3_sym = utils.MNISTNet_b3_sym()
...
#trainer sym
trainer_Sym_basic = utils.Trainer_Sym(utils.mnistbasic_img_transform, test_loader_b_ns,data_dir, model_dir,learning_rate,basic_sym,device,cfg)
#trainer_Sym_b0 = utils.Trainer_Sym(utils.mnistb0_img_transform,train_set_b0_s, test_loader_b0_ns,data_dir, model_dir,learning_rate,batch_size, b0_sym,device,cfg)
#trainer_Sym_b3 = utils.Trainer_Sym(utils.mnistb0_img_transform, test_loader_b3_ns,data_dir, model_dir,learning_rate,batch_size,  b3_sym,device,cfg)
...
if no_train is True:
    ...
    rb_train_s = trainer_Sym_basic.train()
    print(rb_train_s)
    ...
    rb0_train_s = trainer_Sym_b0.train()
    ...
    rb3_train_s = trainer_Sym_b3.train()
...
for model in rows:
    ...
    df.loc[model, "Train (NeSy)"] = fmt_cell(r["train_nesy"]["loss"], max(r["train_nesy"]["accuracy"]), r["train_nesy"]["single-digit accuracy"])
