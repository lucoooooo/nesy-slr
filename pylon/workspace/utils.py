    ...
    def train(self, num_epoch):
        ...
        for epoch in range(num_epoch):
            ...
            for ((d1,d2), target) in tqdm(self.train_loader, total=len(self.train_loader), desc='Train Loop'):
                ...
                ploss = self.loss_pylon(dprob1, dprob2, summation = target)
                dprob1 = F.softmax(dprob1, dim=1)
                dprob2 = F.softmax(dprob2, dim=1)
                output = conv_sum(dprob1, dprob2)
                nllloss = self.loss_nn(torch.log(output + 1e-9), target.long())
                loss = self.lambda_nn * nllloss + self.lambda_pylon * ploss
                loss.backward()
                self.optimizer.step()
